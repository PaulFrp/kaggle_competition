...
k_best = min(25, len(feature_cols))  # Select top 25 features or all if less
selector_kbest = SelectKBest(score_func=f_classif, k=k_best)
selector_kbest.fit(X_train_raw, y_train)
...
for idx, row in feature_mi.head(10).iterrows():
    print(f"       {row['feature']:35s}: {row['mi_score']:.4f}")

# Combine both methods: features in top 30 of either method
selected_features_kbest = feature_scores.head(30)['feature'].tolist()
selected_features_mi = feature_mi.head(30)['feature'].tolist()
selected_features_combined = list(set(selected_features_kbest) | set(selected_features_mi))
...
X_train_selected = X_train_raw.iloc[:, selected_indices]
X_test_selected = X_test_raw.iloc[:, selected_indices]
...
y_train_proba = nb_model.predict_proba(X_train_selected)[:, 1]
...
# ============================================================================
# For Gaussian NB, analyze class-conditional means and variances
feature_importance = []

for i, feature in enumerate(selected_feature_names):
    # Calculate mean difference between classes
    mean_class_0 = X_train_selected[y_train == 0].iloc[:, i].mean()
    mean_class_1 = X_train_selected[y_train == 1].iloc[:, i].mean()
    mean_diff = abs(mean_class_1 - mean_class_0)
    
    # Calculate variance ratio
    var_class_0 = X_train_selected[y_train == 0].iloc[:, i].var()
    var_class_1 = X_train_selected[y_train == 1].iloc[:, i].var()
    var_ratio = max(var_class_1, var_class_0) / (min(var_class_1, var_class_0) + 1e-10)
    
    feature_importance.append({
        'feature': feature,
        'mean_diff': mean_diff,
        'var_ratio': var_ratio,
        'importance_score': mean_diff * (1 + np.log(var_ratio + 1))
    })
...
for idx, row in feature_importance_df.head(15).iterrows():
    print(f"     {row['feature']:35s}: {row['importance_score']:.2f}")

# ============================================================================
# 9. PREDICT ON TEST SET
# ============================================================================
y_test_pred = nb_model.predict(X_test_selected)
y_test_proba = nb_model.predict_proba(X_test_selected)[:, 1]
...
submission.to_csv('/Users/paolacassinelli/Desktop/Foundation of Machine Learning/dsba-m-1-challenge-purchase-prediction/naive_bayes_enhanced_submission.csv', index=False)